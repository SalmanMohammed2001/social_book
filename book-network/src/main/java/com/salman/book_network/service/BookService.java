...
import com.salman.book_network.repo.BookTransactionHistoryRepository;
import com.salman.book_network.util.criteria.BookSpecification;
import com.salman.book_network.util.file.FileStorageService;
import com.salman.book_network.util.file.FileStorageservice;
...
import static com.salman.book_network.util.criteria.BookSpecification.withOwnerId;
...
public class BookService {
    ...
    public PageResponse<BorrowedBookResponse> finAllBorrowedBooks(int page, int size, Authentication connectedUser) {
        User user = ((User) connectedUser.getPrincipal());
        Pageable pageable =  PageRequest.of(page,size, Sort.by("createDate").descending());
    ...
    public PageResponse<BorrowedBookResponse> finAllReturnedBooks(int page, int size, Authentication connectedUser) {
        User user = ((User) connectedUser.getPrincipal());
        Pageable pageable =  PageRequest.of(page,size, Sort.by("createDate").descending());
    ...
    public Integer returnBorrowBook(Integer bookId, Authentication connectedUser) {
        ...
        if(Objects.equals(book.getOwner().getId(),user.getId())){
            throw  new OperationNotPermittedException("You cannot borrow or returned your own books ");
